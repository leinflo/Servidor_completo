...
*/
	...
	strcat(query,aux[1]);//nombre del usuario formato Dominio/usuario
	strcat(query,"','");
	strcat(query,aux[2]);//path completo del archivo
	strcat(query,"','");
...
*/
	...
	query = "select count(*) as existe from Equipos where Direccion_IP = '";
	strcat(query,direccionIP);//direccion IP
	strcat(query,"'");
	...
	SHA(direccionIP,sizeof(direccionIP)-1,digesto);


	strcpy(query,"insert Equipos into values (NULL,'");
	strcat(query,direccionIP);//direccion IP
	strcat(query,"','");
	strcat(query,digesto);//nombre del usuario formato Dominio/usuario
	strcat(query,"','NO','NO')");
...
*/
	...
	strcpy(query,"Select digesto_de_verificacion as digesto from Equipos where Direccion_IP ='");
	strcat(query,direccionIP);//direccion IP
	strcat(query,"'");
